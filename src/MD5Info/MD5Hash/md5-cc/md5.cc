...
/*
	2005
	Updated and changed by A. Bram Neijt <bneijt@gmail.com>
	
	This class is very old and very ugly. This can't even be 
	called C++ if you ask me.
	
	I've changed some of the interface functions but left the rest
	to rot. Note that some of the functions mentioned here arn't even
	implemented.
	
	One day, I'll purge this ugly code and write my own ;-)
...
void MD5::update(istream& stream){
  ...
  while (stream.good()){
    stream.read(reinterpret_cast<char *>(&buffer[0]), 1024); // note that return value of read is unusable.
    len=stream.gcount();
    update(buffer, len);
  }
